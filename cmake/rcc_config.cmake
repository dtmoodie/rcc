...
macro(strip_lib_ext IN_VAR OUT_VAR)
    IF(WIN32)
        get_filename_component(${OUT_VAR} ${${IN_VAR}} NAME_WE)
    else(WIN32)
        get_filename_component(tmp ${${IN_VAR}} NAME)
        string(FIND ${tmp} ".so" IDX)
        if(IDX GREATER 0)
            string(SUBSTRING "${tmp}" 0 ${IDX} ${OUT_VAR})
        else()
            string(FIND ${tmp} ".a" IDX)
            string(SUBSTRING "${tmp}" 0 ${IDX} ${OUT_VAR})
        endif()
    endif(WIN32)
endmacro()
...
macro(RCC_WRITE_TARGET_CONFIG target inc_dirs lib_dirs flags defs)
    ...
    list(APPEND defs ${defs_})
    get_target_property(defs_ ${target} INTERFACE_COMPILE_DEFINITIONS)
    list(APPEND defs ${defs_})

    get_directory_property(defs_ DIRECTORY ${CMAKE_CURRENT_LIST_DIR} COMPILE_DEFINITIONS)
    list(APPEND defs ${defs_})
...
endmacro(RCC_WRITE_TARGET_CONFIG)
...
macro(RCC_TARGET_CONFIG target LIB_FILES_DEBUG_VAR LIB_FILES_RELEASE_VAR)
    ...
    RCC_WRITE_TARGET_CONFIG(${target} ${inc_dirs} ${lib_dirs} ${flags} ${defs})
