...
namespace detail
{
    ...
    {
        return len == 0 ? 0xFFFFFFFF : ctCombine_crc32(str, ctcrc32(str, len - 1), len);
    }

    template<size_t idx>
    constexpr size_t findDeliminator(const char* str)
    {
        return str[idx] == ':' ? idx : findDeliminator<idx - 1>(str);
    }

    template<>
    constexpr size_t findDeliminator<size_t(0)>(const char* str)
    {
        return 0;
    }
...
{
    return detail::findDeliminator<Tlen - 1>(str);
}

constexpr size_t classNameIdxLen(const char* str, int len)
{
    return detail::ctFindDeliminator(str, len);
}

template<size_t Tlen>
constexpr uint32_t hashClassName(const char(&str)[Tlen]) {
    return ctcrc32Range(str, classNameIdxLen(str, Tlen - 1));
}
...
struct IObject
{
    static const InterfaceID s_interfaceID = ctcrc32("IObject");
